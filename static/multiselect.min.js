if (!m_helper) {
    var m_helper = {
        ...
        hideAll: function (array) {
            for (var i = 0; i < array.length; i++) { this.hide(array[i]) }
        },
        show: function (element) {
            element.style.display = 'block'
        },
        showAll: function (array) {
            for (var i = 0; i < array.length; i++) { this.show(array[i]) }
        },
...
Multiselect.prototype =
{
    ...
    _initIsEnabled: function () { this.setIsEnabled(!this._item.disabled) },
    destroy() {
        m_helper.removeNode(this._getIdentifier());
        ...
        if (val) {
            m_helper.each(document.getElementById(this._getIdentifier()).querySelectorAll('.multiselect-checkbox'), function (e) {
                if (e.dataset.val == val) {
                    if (setCheck && !e.checked) { m_helper.check(e); self._onCheckBoxChange(e, self) }
    ...
    setCheckBoxClick(id, handler) {
        if (typeof handler === "function") { this._checkboxClickEvents[id] = handler }
    ...
    _appendEvents: function () {
        ...
        document.getElementById(self._getInputFieldIdentifier()).addEventListener('focus', function (event) {
            ...
            m_helper.each(window.multiselects, function (e) {
                if (document.getElementById(e._getItemListIdentifier()).offsetParent && m_helper.parent(event.target, e._getIdentifier())) { e._hideList(self) }
            })
        });
        document.getElementById(self._getInputFieldIdentifier()).addEventListener('click', function () { self._showList(self); document.getElementById(self._getInputFieldIdentifier()).value = '' });
        ...
        m_helper.each(document.getElementById(self._getIdentifier()).querySelectorAll('.multiselect-checkbox'), function (e) {
            e.addEventListener('change', function (event) { self._onCheckBoxChange(e, self, event) })
        });
        var onInput = function () {
            ...
            if (!text || text == '') {
                ...
                var array = Array.prototype.filter.call(document.querySelectorAll('#' + self._getItemListIdentifier() + ' li span'), function (obj) { return obj.innerHTML.toLowerCase().indexOf(text) > -1 });
    ...
    _onCheckBoxChange: function (checkbox, self, event) {
        if (!checkbox.dataset.multiselectElement) {
            ...
            if (typeof self._checkboxClickEvents.checkboxAll === "function") { self._checkboxClickEvents.checkboxAll(checkbox, { checked: checkedState }) }
        }
        else {
    ...
    _updateText: function (context) {
        ...
        if (activeItems.length > 0) {
            var val = '';
            for (var i = 0; i < (activeItems.length < 5 ? activeItems.length : 5); i++) { val += activeItems[i].innerText + ", " }
...
function hideMultiselects(event) {
    m_helper.each(window.multiselects,
        function (e) {
            if (document.getElementById(e._getItemListIdentifier()).offsetParent && !m_helper.parent(event.target, e._getIdentifier())) { e._hideList(e, event) }
        })
}